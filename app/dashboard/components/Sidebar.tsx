...
export const Sidebar = ({user} : {user?: User} ) => {
    ...
    useEffect(() => {
        ...
        useEffect(()    => {
            console.log("UESR FOUND:", user)
}, [user]);

return (
    <>
        <button onClick={() => setSidebarOpen(!sidebarOpen)} data-drawer-target="settings-menu" data-drawer-toggle="settings-menu" aria-controls="settings-menu" type="button" className="inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600">
                        ...
                        <ul id="user-settings" className={`${settingsOpen ? 'opacity-100 h-16' : 'opacity-0 h-0'} pl-2 transition-all duration-300 ease-in-out flex flex-col gap-4`}>
                            ...
                            <li>
                                <div className="indicator flex flex-row no-wrap">
                                    ...
                                    <div className="text-small text-content"> Configure Log Groups</div>                  
                                </div>
                            </li>
                        </ul>
                    </li>
                    <li className="mt-4 border-t border-gray-200 dark:border-gray-700">
